...
$BODY$
    WITH ins1 AS (
        ...
        SELECT max(song_id), artistid FROM mediaserver.song
        )
        INSERT INTO mediaserver.MediaItemMetaData
        SELECT media_id, md_id FROM ins1, ins2;
...
create or replace function mediaserver.insertUserMediaConsumption(
    inUsername varchar(50),
    inMedia_id INTEGER,
    inPlay_count INTEGER,
    inProgress FLOAT,
    inLastviewed DATE)
RETURNS void AS
$$
		INSERT INTO mediaserver.UserMediaConsumption(username, media_id,play_count, progress,lastviewed)
		VALUES(inUsername, inMedia_id,inPlay_count, inProgress,inLastviewed);
$$
LANGUAGE sql;

CREATE OR REPLACE FUNCTION mediaserver.updateUserMediaConsumption(
	inUsername varchar(50),
    inMedia_id INTEGER,
    inPlay_count INTEGER,
    inProgress FLOAT,
    ...
    BEGIN
        UPDATE mediaserver.UserMediaConsumption
		    SET play_count = inPlay_count,
			progress = inProgress,
			lastviewed = inLastviewed
		    WHERE username = inUsername and media_id = inMedia_id;
    END;
$$ LANGUAGE plpgsql;
